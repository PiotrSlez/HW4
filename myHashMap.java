...
class myHashMap<K,V> {
    ...
    public V remove(K key) {
        ...
        V oldValue = get(key); //stored for return

        int index = getBucketIndex(key); //finds bucket
        HashNode<K,V> head =  bucket.get(index);

        while(head != null){
            if(head.key.equals(key)){
                bucket.set(index, head.next); //sets current node to next node
                size--;
            }
            head = head.next; //iterates through bucket
        }
    ...
    public V replace(K key, V val) {
        ...
        V oldVal = get(key); //stored for return

        int index = getBucketIndex(key); //finds bucket
        HashNode<K,V> head = bucket.get(index);

        while(head != null) { //traverses bucket
            if (head.key.equals(key)) {
                head.value = val; //changes value of val if found key
            }
            head = head.next;
        }
    ...
    public boolean replace(K key, V oldVal, V newVal) {
        ...
        if(get(key) == oldVal){ //replaces key with newVal if oldVal is correct
            replace(key, newVal); //use of replace(K,V)
            return true;
        }
